...
func (h *DNSHandler) do(config *Config, blockCache *MemoryBlockCache, questionCache *MemoryQuestionCache) {
	for {
		...
		func(Net string, w dns.ResponseWriter, req *dns.Msg) {
			...
			mesg, err := h.resolver.Lookup(Net, req, config.Timeout, config.Interval, config.Nameservers, config.DoHs)
			...
			if mesg.Truncated && Net == "udp" {
				mesg, err = h.resolver.Lookup("tcp", req, config.Timeout, config.Interval, config.Nameservers, config.DoHs)
				if err != nil {
